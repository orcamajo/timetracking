...
class AggregationService {
     ...
     */
    aggregateWorklogs(roots, worklogs) {
    ...
    attachWorklogs(nodes, worklogMap) {
     ...
     */
    aggregateWorklogsDailyBreakdown(roots, worklogs) {
        ...
        const convert = (node) => {
            ...
            const authors = Array.from(authorMap.entries()).map(([accountId, a]) => ({
                accountId,
            ...
            const rollupDailySeconds = { ...dailySeconds };
    ...
    rollup(node) {
        ...
        for (const child of node.children) {
            ...
            for (const [accountId, seconds] of Object.entries(child.rollupTimeByPerson)) {
